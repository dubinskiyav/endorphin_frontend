...
import { Button, Menu, Dropdown, Form, Space, notification,Radio } from 'antd';
...
import { MoreOutlined, ClockCircleOutlined, ExclamationCircleOutlined, CopyOutlined,DollarOutlined,CloseCircleOutlined } from '@ant-design/icons';
import { drawDate, drawFloat, drawInt, drawStatus, drawDateAndTime,getItemFromLocalStorage, refreshStatusList } from "../../lib/Utils";
...
const COLUMNS = [
    {
        ...
        render: (_, record) => <>{drawStatus(_, record)}
            {record.requestagreePaidflag?
                <div className={"paid-badge"}>оплачено</div>:""}</>
    },
    {
        title: 'Номер',
...
const buildFilters = (statuses) => {
    return <React.Fragment>
        <Primary>
            ...
            <Radio.Group key="paidFlag" buttonStyle="solid">
              <Radio.Button value="-1">Все</Radio.Button>
              <Radio.Button value="0">Без оплаты</Radio.Button>
              <Radio.Button value="1">Оплаченные</Radio.Button>
            </Radio.Group>
        </Primary>
        <Space direction="vertical">
            <Space>
...
const paid = (paidOn,tableInterface)=>{
    let ids = tableInterface.getSelectedRows();
    confirm(paidOn ? `Установить флаг "Оплачено" у ${ids.length} заказов?` : `Снять флаг оплаты у ${ids.length} заказов?`, () => {
        requestToAPI.post("tentorium/documents/requestin/request/"+(paidOn?"uppaidflag":"downpaidflag"), ids)
            .then(() => {
                tableInterface.refreshData();
                notification.success({ message: paidOn ? "Флаг \"Оплачено\" успешно установлен " : "Флаг \"Оплачено\" успешно снят" })
            })
            .catch(error => {
                notification.error({
                    message: paidOn ? "Ошибка при установке флага Оплачено" : "Ошибка при снятии флага Оплачено",
...
const buildMenuCommand = (config, handleMenuClick) => {
    const tableInterface = config.tableInterface;
    return <Menu onClick={handleMenuClick}>
        <Menu.Item key="paid-on-mnu" onClick={() => paid(true,tableInterface)} icon={<DollarOutlined />}
                disabled={!tableInterface.isLoading || tableInterface.isLoading() || tableInterface.getSelectedRows().length == 0}>Установить флаг Оплачено</Menu.Item>
        <Menu.Item key="paid-off-mnu" onClick={() => paid(false,tableInterface)} icon={<CloseCircleOutlined />}
            disabled={!tableInterface.isLoading || tableInterface.isLoading() || tableInterface.getSelectedRows().length == 0}>Снять флаг оплаты</Menu.Item>
        <Menu.Divider/>
...
const RequestIn = (props) => {
    ...
    const menuCommand = buildMenuCommand({tableInterface},buildMenuHandler({
        'topLayer': topLayer,

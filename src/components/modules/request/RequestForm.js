...
import { Form, Checkbox, Input, Col, Row, Button, Popover, Space } from 'antd';
...
import { requestPosGroupForm } from './RequestPosGroupForm';
...
const RequestForm = (props) => {
    ...
    const [numberingInterface] = React.useState({});
    ...
    // проверка наличия введенных значений Принято
    const receiveValuesFound = React.useCallback(() =>{
        if(!tableInterface.memoryDataSet) return false;
        return tableInterface.memoryDataSet.data
            .findIndex(val => val.record.requestPosCountAccept>0) >= 0;
    },[tableInterface]);
    // Принято полностью
    const isFullAccepted = (rec)=>(parseInt(rec.requestPosCountAccept)===rec.requestPosCount);

    const drawAcceptedValue = (record,value)=>
        <div style={!isFullAccepted(record)?{ color: "red" }:{}}>{value}</div>

    // сверка принятого количества с плановым. Только для отгруженного заказа
    const isEqualsTotalCountAndCountAccept = ()=>{
        if(requestType==REQUEST_ACCEPTED && receiveValuesFound()) {
            return tableInterface.memoryDataSet.data
                            .findIndex(val=>parseInt(val.record.requestPosCountAccept)!=val.record.requestPosCount)<0;
        }
        return true;
    }

    const refreshTotal = React.useCallback(()=>{
        if (tableInterface.getScalarSumField) {
            const countFldName = (requestType==REQUEST_ACCEPTED && receiveValuesFound())?"requestPosCountAccept":"requestPosCount";            
            props.form.setFieldsValue({
                sum: drawFloat(tableInterface.getScalarSumField("requestPosPrice", countFldName)),
                points: drawInt(tableInterface.getScalarSumField("requestPosPoints", countFldName)),
                weight: drawFloat(tableInterface.getScalarSumField("requestPosWeight", countFldName), undefined, undefined, 3),
            })
        }
    },[props.form,receiveValuesFound,requestType,tableInterface]);

    props.internalTrigger.onAfterLoad=()=>{
        refreshTotal();
    }
    ...
    const buttons = [
        ...
        <Button key="addGroup"
            type="primary"
            onClick={() => requestPosGroupForm({
                priceType: 6,
                priceTypeForPoints: 9,
                dateForPrice: moment(props.form.getFieldValue("documentRealDate"))
                    ? moment(props.form.getFieldValue("documentRealDate")).valueOf()
                    : null,
                subjectId: isProvider ? userProps.subject.subjectId : userProps.parent.subjectId,
            }, (selected, records) => {
                if (!selected) return;
                const oldUpdateRecords = updateRecords;
                records.forEach(value => {
                    if (tableInterface.memoryDataSet.data.filter(val => val.record.characterId === value.characterId).length === 0) {
                        afterAdd(value);
                    } else {
                        const foundRecord = tableInterface.memoryDataSet.data.filter(val => val.record.characterId === value.characterId)[0].record;
                        const oldCount = value.requestPosCount;
                        Object.assign(value, foundRecord);
                        value.requestPosCount = oldCount + foundRecord.requestPosCount;
                        value.requestPosId = foundRecord.requestPosId;
                        afterEdit(value);
                    }
                })
                setUpdateRecords([...new Set([...oldUpdateRecords, ...records.map(value => transformValues(value))])]);
            }, tableInterface.memoryDataSet.data)} disabled={fieldDisabled} hidden={requestType !== REQUEST_DRAFT}>Добавить группу товаров</Button>,
    ...
    const getColumns = () => {
        ...
        let COLUMNS = [
            ...
            },
            {
                ...
                align: "right",
                width: "70px",
                editable: (requestType === REQUEST_DRAFT) && isCustomer,
            ...
            },
            {
                ...
                render: (val, record) => {
                    if(requestType==REQUEST_ACCEPTED && record["requestPosCountAccept"]){
                        return drawAcceptedValue(record,val, record);
                    } else {
                        return drawInt(val, record);
                    }
                },
                align: "right",
                width: "80px"
                ...
                render: (_, record) => {
                    switch (requestType) {
                        ...
                        case REQUEST_ACCEPTED:
                            if(record["requestPosCountAccept"]) {
                                return drawAcceptedValue(record,drawFloat(record["requestPosCountAccept"] * record["requestPosPrice"], record));
                            }
                            return drawFloat(record["requestPosCount"] * record["requestPosPrice"], record);
            ...
            },
            {
                ...
                render: (_, record) => {
                    switch (requestType) {
                        ...
                        case REQUEST_ACCEPTED:
                            if(record["requestPosCountAccept"]) {
                                return drawAcceptedValue(record,record["requestPosCountAccept"] * record["requestPosPoints"]);
                            }
                            return drawInt(record["requestPosCount"] * record["requestPosPoints"], record);
            ...
            },
            {
                ...
                render: (_, record) => {
                    switch (requestType) {
                        ...
                        case REQUEST_ACCEPTED:
                            if(record["requestPosCountAccept"]) {
                                return drawAcceptedValue(record,drawFloat(record["requestPosCountAccept"] * record["requestPosWeight"], record, undefined, 3));
                            }
                            return drawFloat(record["requestPosCount"] * record["requestPosWeight"], record, undefined, 3);
    ...
    const InputSummary= ()=><Input className={"inputSummary"+(!isEqualsTotalCountAndCountAccept()?" red-text":"")} readOnly={true}/>;
    ...
    return <Form
            ...
            <Col span={8}>
                <Form.Item
                    ...
                    <Numbering
                        ...
                        afterLoad={() => {
                            if (modeAdd && !props.initialValues["documentRealNumber"]) {
                                numberingInterface.generateNumber();
                            }
                        }}
                    />
                </Form.Item>
        ...
        >
            <Space>
                <Input.Search
                    ...
                    onSearch={search => {
                        const usearch = search ? search.toUpperCase() : undefined;
                        const isearch = search ? parseInt(search) : undefined;
                        setSearchFunc(search ? {
                            fc: (r) => {
                                return (r.characterCode && r.characterCode == isearch) ||
                                    (r.sgood && r.sgood.title && r.sgood.title.toUpperCase().indexOf(usearch) >= 0);
                            }
                        } : {});
                        forceUpdate();
                    }}
                ...
                {sumText ? (
                    <Popover content={sumText}>
                        <Form.Item
                            ...
                            labelCol={{ span: 8 }}
                            wrapperCol={{ span: 12 }}>
                            <Input className="inputSummary" readOnly={true} />
                        </Form.Item>
                    </Popover>
                ) : (
                    <Form.Item
        ...
        <Form.Item
            ...
            <MemoryDataTable className="mod-main-table"
                ...
                getRowClass={(record) => (requestType === REQUEST_APPROVAL && record["requestPlanPosCount"] === undefined) ? "newRecord" : ""}
            />
        </Form.Item>
        <EditForm
            id={EDIT_FORM_ID}
            ...
            onShowMessage={(response, resolve) => {
                // Выведем сообщение с подтверждением редактирования
                const foundRecord = tableInterface.memoryDataSet.data.filter(val => {
                    return val.record.characterId === response.sgood.additional.characterId && val.record[ID_NAME] !== editorContext.id
                });
                if (foundRecord.length !== 0) {
                    confirm(format("Товар с характеристикой {0} уже включен в позицию №{1}. Будет увеличено количество товара по данной позиции. Продолжить?",
                        response.sgood.additional.characterCode, foundRecord[0].record.requestPosNumber), () => resolve());
                } else {
                    resolve();
                }
            }}
            afterSave={(response) => {
                ...
                if (response) {
                    if (!editorContext.id) {
                        if (tableInterface.memoryDataSet.data.filter(val => val.record.characterId === response.sgood.additional.characterId).length === 0) {
                            afterAdd(response);
                        } else {
                            const foundRecord = tableInterface.memoryDataSet.data.filter(val => val.record.characterId === response.sgood.additional.characterId)[0].record;
                            const oldCount = response.requestPosCount;
                            Object.assign(response, foundRecord);
                            response.requestPosCount = oldCount + foundRecord.requestPosCount;
                            response.requestPosId = foundRecord.requestPosId;
                            afterEdit(response);
                        }
                    } else {
                        if (tableInterface.memoryDataSet.data.filter(val => {
                            return val.record.characterId === response.sgood.additional.characterId && val.record[ID_NAME] !== editorContext.id
                        }).length === 0) {
                            afterEdit(response);
                        } else {
                            // Удалим запись, которую меняем
                            tableInterface.deleteRecord(Object.assign({}, response));
                            // Найдем и изменим запись с таким же товаром
                            const foundRecord = tableInterface.memoryDataSet.data.filter(val => val.record.characterId === response.sgood.additional.characterId)[0].record;
                            const oldCount = response.requestPosCount;
                            Object.assign(response, foundRecord);
                            response.requestPosCount = oldCount + foundRecord.requestPosCount;

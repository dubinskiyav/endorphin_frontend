...
const MOD_TITLE = "Роли ролей";
const MODE_HELP_ID = "/help/accessrolerole";
const CONTOUR = CONTOUR_ADMIN;
const MODULE = MODULE_CREDENTIAL;

// Сущность (в CamelCase)
const ENTITY = "AccessRoleRole";
...
const COLUMNS = [
    {
        title: 'Роль',
        dataIndex: 'accessRoleNameParent',
        sorter: true,
        ellipsis: true,
...
const buildForm = (form) => {
    return <AccessRoleRoleForm form={form} initialValues={{}} />
}
    ...
    return (
        <App subsystem={MNU_SUBSYSTEM} menu={MNU_MENU} submenu={MNU_SUMMENU}
            ...
            <DataTable className="mod-main-table"
                ...
                editCallBack={(record) => callForm(record.accessRoleRoleId)}
                interface={tableInterface}
                onSelectedChange={() => forceUpdate()}
                onAfterRefresh={() => setUpdateRecords([])}
                updateRecords={updateRecords}
                recordMenu={recordMenu}
                idName="accessRoleRoleId"
            />
            <EditForm
                id={EDIT_FORM_ID}
                copyButtonFlag={true}
...
export default withRouter(AccessRoleRole);
